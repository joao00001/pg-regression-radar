...
| `--dashboard-addr` | `` | Listen address for the read-only dashboard API (`GET /api/v1/{regressions,deploys,queries,watches}` — see [API Reference](api-reference.md#dashboard-api)). Empty (default) disables it. `/api/v1/deploys` and `/api/v1/queries` work regardless of `--state-backend`. `/api/v1/regressions` and `/api/v1/watches` always answer `501 Not Implemented` on this binary, since `cmd/operator` has no Kubernetes client; run `cmd/manager` (`--dashboard-addr` there too) for those two routes |
...
| `--dashboard-addr` | `` | Listen address for the read-only dashboard API (`GET /api/v1/{regressions,watches}` — see [API Reference](api-reference.md#dashboard-api)). Empty (default) disables it. Unlike `--webhook-bind-address`/`--pg-metrics-bind-address`, served on every replica (not leader-only), since it only reads the Kubernetes API server directly. `/api/v1/deploys` and `/api/v1/queries` always answer `501 Not Implemented` on this binary; run `cmd/operator` for those two routes |
...
| `dsn` / `dsnSecretRef` | *(one required)* | Postgres DSN, inline or via a Secret key (preferred). **Use a least-privilege role** (e.g. `pg_monitor`) — see [Installation: least-privilege role](installation.md#creating-a-least-privilege-postgres-role). The referenced Secret must carry the `pg-regression-radar.io/allow-postgreswatch-access: "true"` label or the manager refuses to read it — see [Multi-Cluster: Secret consent label](multi-cluster.md#secret-consent-label). In multi-tenant clusters, also enforce who can set that label via admission policy (example: [`docs/policies/kyverno-consent-label-policy.yaml`](policies/kyverno-consent-label-policy.yaml)). |
| `remoteClusterRef` | *(none)* | Preferred remote-cluster path: name of a pre-registered `PostgresRadarCluster` (cluster-scoped). The manager reads that registry entry's kubeconfig Secret and resolves `dsnSecretRef` through the referenced remote cluster — see [Multi-Cluster (Fleet) Mode](multi-cluster.md). |
| `remoteClusterSecretRef` | *(none)* | **Deprecated** direct Secret path: Secret (in the hub cluster) holding a kubeconfig for a remote cluster to resolve `dsnSecretRef` against instead of the hub. Still accepted in `controlled` security-profile mode for backward compatibility, rejected in `hardened` mode. Same consent-label requirement as `dsnSecretRef` above, and the kubeconfig itself must use a static credential (no `exec`/`auth-provider`/`proxy-url`) — see [Multi-Cluster: Kubeconfig restrictions](multi-cluster.md#kubeconfig-restrictions). |
| `remoteNamespace` | *(same name as the watch's own namespace)* | Namespace to look up `dsnSecretRef` in on the remote cluster; meaningful alongside either `remoteClusterRef` or `remoteClusterSecretRef` — see [Multi-Cluster (Fleet) Mode: What actually happens on reconcile](multi-cluster.md#what-actually-happens-on-reconcile) |
...
| `alerting.destinationPolicy` | `""` (→ `permissive`) | Destination policy override for this watch: `permissive`, `allowlist`, or `relay-only`. Used only when manager-level `--alerting-destination-policy` is left at default/empty — see [Alerting: destination policies](alerting.md#destination-policies) |
