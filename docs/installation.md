...
```
> **Note:** The DSN values above are placeholders. Replace `“******host:5432/dbname?sslmode=disable”`
> (and any similar redacted DSN in this documentation) with a full PostgreSQL connection URI such as
> `******host:5432/dbname?sslmode=disable`.
> Redacted examples are shown for documentation safety only and cannot be used as-is.
...
> connection URI, for example `******cnpg-cluster-rw.production:5432/mydb?sslmode=disable`.
