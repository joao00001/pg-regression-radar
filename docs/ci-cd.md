...
| `release-notes` (needs `images`, `helm-chart`) | Generates release notes with [git-cliff](https://git-cliff.org) (config: `cliff.toml` at the repo root) from the Conventional Commits between the previous tag and the one just pushed, then publishes a real GitHub Release on that tag with those notes as the body, via [`softprops/action-gh-release`](https://github.com/softprops/action-gh-release). Runs last (after both publish jobs) so a release page never advertises artifacts that failed to build, scan, or push. |
...
**Changelog generation (`release-notes` job):** `git-cliff` was chosen over heavier alternatives like `release-please`/`semantic-release` specifically because it doesn't take over version decisions — it just renders Conventional Commits already on `git log` into notes for a tag the maintainer already decided to push, via a single Rust binary (no bot-owned "release PR" to merge first). `cliff.toml`'s `commit_parsers` mirror CONTRIBUTING.md's type table exactly (`feat`/`fix`/`perf`/`docs`/`refactor`/`test`/`ci`/`chore`), plus a catch-all `Other` group so a not-yet-conventional commit (this repo's history predates `pr-title.yml` enforcement) is demoted rather than silently dropped. The same config also regenerates the repo-root `CHANGELOG.md` by hand from time to time — see that file's own header.

**Not yet proven green in CI:** like `e2e-kind.yml` before its first real `workflow_dispatch` run, this workflow's commands were checked against current upstream `docker/build-push-action`, `docker/login-action`, `aquasecurity/trivy-action`, `anchore/sbom-action`, `sigstore/cosign-installer`, `orhun/git-cliff-action`, and `softprops/action-gh-release` documentation, and the YAML is syntax- and `actionlint`-clean, but the environment this was authored in had no Docker daemon at all (not even a local `docker build` of the five targets could be exercised) and obviously no real tag push against the real `ghcr.io` registry or a real GitHub Release creation. `git-cliff` itself *was* run for real against this repo's actual commit history (a downloaded pre-built binary, not compiled) to validate `cliff.toml` produces a correct, complete changelog — see [Roadmap](roadmap.md#operational-follow-ups) — but the `release-notes` job's use of it inside an Actions runner, and the `softprops/action-gh-release` call, are unproven the same way the rest of this workflow is. Treat the first real release as part of reviewing the change that introduced this workflow — see [Roadmap](roadmap.md#operational-follow-ups).
